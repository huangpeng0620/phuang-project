...
/**
 *
 * @description rag 查询改写器
 * <p>
 * 基于 LLM 对用户查询进行智能改写优化，结合历史对话上下文，提升 RAG 检索效果,支持五种改写策略:
 * <ul>
...
public class KnowEngineQueryTransformer implements QueryTransformer {
    ...
    /**
     * Spring 容器对象, 由使用方在构造时传入
     * <P>
     *     volatile 保证一个线程写入后，其他线程能够立即看到最新值,因为问题改写和结果回写可能运行在不同线程，尤其下面还启动了虚拟线程;
     *     static 关键字保证所有 KnowEngineQueryTransformer 实例共享同一个 applicationContext Bean
     *     可以通过调用 ApplicationContext 的 getBean 方法获取任意一个 spring 管理的对象
     *   KnowEngineApplication 启动类启动时会通过调用 setApplicationContext 方法将真实的 spring 容器对象赋值给这个静态变量
     * 完整流程:
     *             Spring 应用启动
     *                 ↓
     *             创建 ApplicationContext
     *                 ↓
     *             回调 KnowEngineApplication.setApplicationContext()
     *                 ↓
     *             调用 KnowEngineQueryTransformer.setApplicationContext()
     *                 ↓
     *             静态保存 Spring 容器
     *   保存后就能通过下面的 getChatMessageService 方法动态获取 Bean
     * 为什么不能使用 @Autowired ?
     *   因为 KnowEngineQueryTransformer 不是 Spring Bean，它没有 @Component，而是在业务代码中通过 new 关键字手动创建的;
     * </P>
     */
     ...
     * 获取 chat 消息服务类 Bean
     * <P>
     *     由于当前类没有被Spring托管,无法通过注入的方式获取到 ChatMessageService Bean
     * </P>
     * @return
     */
    ...
    public Collection<Query> transform(Query query) {
        ...
        return List.of(compressedQuery);
    }
