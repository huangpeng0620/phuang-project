...
public class FileStorageService {
    ...
    private void createBucketIfNotExists(boolean publicRead) throws Exception {
        ...
        // 设置 bucket 策略为公共读 (使得上传图片后的url可以公开访问)
        if (publicRead) {
    ...
    public String uploadFile(String objectName, byte[] content, String contentType) throws Exception {
        try (InputStream stream = new ByteArrayInputStream(content)) {
            return uploadFile(objectName, stream, content.length, contentType);
        }
    }

    /**
     * 通过输入流上传文件，输入流由调用方负责关闭
     */
    public String uploadFile(String objectName, InputStream inputStream, long objectSize, String contentType)
            throws Exception {
        createBucketIfNotExists(true);
