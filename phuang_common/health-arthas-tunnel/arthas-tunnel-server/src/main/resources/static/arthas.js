...
$(document).ready(function () {
    document.title = "artahs tunnel server";
...
function reloadRegisterApplications() {
    var result = reqSync("/arthas/access/agents", "get");
...
function initServerInfo() {
    var result = reqSync("/arthas/server", "get");
    serverInfo = result;
...
function initArthasAgentSplit() {
    var result = reqSync("/arthas/agent/split", "get");
...
function selectServiceOnchange(service) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        if (agent.envInfo != '' && agent.envInfo != null) {
            var text = agent.info.host + ':' + agent.envInfo;
        } else {
            var text = agent.info.host + '_' + agent.info.port;
        }
